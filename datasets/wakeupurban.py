...
def get_wu_labeldata():
    cls_names = ['buildings','road','water','green','others']
    colormap = np.array([
            [0, 255, 0], #[101, 192, 0],[0, 130, 122],
...
class wakeupurban(Dataset):
    def __init__(self, transforms, split, root, datafrom = 'image'):
        super(wakeupurban, self).__init__()
        self.split = split
        self.root = root
    ...
    def __getitem__(self, index):
        ...
        base_seg = Image.open(join(self.root.replace('/wakeupurban', ''), "SAMoutput_gts_base", image_id + ".png"))
...
classes =  ['buildings','road','water','green','others']
